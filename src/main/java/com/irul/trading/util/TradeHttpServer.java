        ...
        public void handle(HttpExchange exchange) throws IOException {
            if ("POST".equals(exchange.getRequestMethod())) {
                ...
                try {
                    ...
                    // #region agent log
                    DebugLogger.log("pre-fix", "H3", "TradeHttpServer.java:MarketDataHandler",
                        "MT5 market POST received", new JSONObject()
                            .put("bodyLength", body.length())
                            .put("action", action)
                            .put("hasSymbol", obj.has("symbol"))
                            .put("hasBid", obj.has("bid"))
                            .put("hasAsk", obj.has("ask")));
                    // #endregion
                    if ("MARKET".equals(action)) {
                        String symbol = obj.getString("symbol");
                        double bid = obj.getDouble("bid");
                        double ask = obj.getDouble("ask");
                        boolean saved = DatabaseHelper.saveMarketData(symbol, bid, ask);
                        // #region agent log
                        DebugLogger.log("pre-fix", "H5", "TradeHttpServer.java:MarketDataHandler",
                            "Market tick save attempt", new JSONObject()
                                .put("symbol", symbol)
                                .put("bid", bid)
                                .put("ask", ask)
                                .put("saved", saved));
                        ...
                        // #region agent log
                        DebugLogger.log("pre-fix", "H4", "TradeHttpServer.java:MarketDataHandler",
                            "Rejected market payload", new JSONObject().put("action", action));
                        // #endregion
                        exchange.sendResponseHeaders(400, -1);
                    }
                } catch (Exception e) {
                    // #region agent log
                    DebugLogger.log("pre-fix", "H4", "TradeHttpServer.java:MarketDataHandler",
                        "Market handler exception", new JSONObject().put("error", e.getMessage()));
                    // #endregion
                    e.printStackTrace();
                    exchange.sendResponseHeaders(500, -1);
                }
