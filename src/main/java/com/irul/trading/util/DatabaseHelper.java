...
public class DatabaseHelper {
     ...
     * @param ask    Harga ask
     */
    public static boolean saveMarketData(String symbol, double bid, double ask) {
        ...
        try (Connection conn = getConnection();
            ...
            // #region agent log
            DebugLogger.log("pre-fix", "H1", "DatabaseHelper.java:saveMarketData",
                "Market row inserted", new JSONObject()
                    .put("dbUrl", DB_URL)
                    .put("symbol", symbol)
                    .put("bid", bid)
                    .put("ask", ask)
            ...
            // #region agent log
            DebugLogger.log("pre-fix", "H5", "DatabaseHelper.java:saveMarketData",
                "Market insert failed", new JSONObject()
                    .put("dbUrl", DB_URL)
                    .put("symbol", symbol)
    ...
    public static double getLatestAsk(String symbol) {
        ...
        try (Connection conn = getConnection();
            ...
            if (rs.next()) {
                ...
                // #region agent log
                DebugLogger.log("pre-fix", "H2", "DatabaseHelper.java:getLatestAsk",
                    "Exact symbol match found", new JSONObject()
                        .put("dbUrl", DB_URL)
                        .put("symbol", symbol)
                        .put("ask", ask));
                // #endregion
            ...
            String likeSummary = getLatestMarketSummaryByLike(
                symbol.replaceAll("m$", "").replaceAll("M$", ""));
            // #region agent log
            DebugLogger.log("pre-fix", "H2", "DatabaseHelper.java:getLatestAsk",
                "No exact symbol match", new JSONObject()
                    .put("dbUrl", DB_URL)
                    .put("symbol", symbol)
                    .put("likeFallback", likeSummary));
            // #endregion
        } catch (SQLException e) {
            // #region agent log
            DebugLogger.log("pre-fix", "H1", "DatabaseHelper.java:getLatestAsk",
                "Query failed", new JSONObject()
                    .put("dbUrl", DB_URL)
                    .put("symbol", symbol)
                    .put("error", e.getMessage()));
            // #endregion
